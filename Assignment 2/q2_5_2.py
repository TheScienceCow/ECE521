...
def train(miniBatchSize,learningRate,weightDecay,trainingEpochs, keepProb):
    ...
    for i in range(num_layers): #drop-out between EVERY layer.
        print("Hidden layer {0}, with {1} HU".format(i,num_HUs[i]), file =f)
        z_h, h_weights = layerFunc(activation_last,dim_last,num_HUs[i],i)
...
for i in range(len(keepProbs)):
    ...
    u, vll, trll, tell, teal, tral, val = train(miniBatchSize, float(learning_rate[i]),float(weight_decay[i]),trainingEpochs,float(keepProbs[i]))
