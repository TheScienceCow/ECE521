...
def train(miniBatchSize,learningRate,weightDecay,trainingEpochs,numHiddenUnits):
    print("Starting Training with parameters MBS {0}, LR {1}, WD {2}, TE {3}, NHU {4}, numLayers = 2".format(miniBatchSize,learningRate,weightDecay,trainingEpochs,numHiddenUnits), file = f)
    #load the dataset
    ...
    z_h2, h2_weights = layerFunc(tf.nn.relu(z_h),numHiddenUnits,numHiddenUnits,0)
    h2 = tf.nn.relu(z_h2)

    out, out_weights = layerFunc(h2,numHiddenUnits,10,1)
...
training_CE_all = []
training_acc_all = []
u, vll, trll, tell, teal, tral, val = train(miniBatchSize, learning_rate,weight_decay,trainingEpochs,numHiddenUnits)
plt.figure()
...
plt.title("Cross-entropy vs. Epochs for two-hidden layers of {0} units".format(numHiddenUnits))
plt.legend()
plt.figure()
...
plt.xlabel("Epochs")
plt.ylabel("Accuracy")
plt.title("Accuracy vs. Epochs for two-hidden layers of {0} units".format(numHiddenUnits))
plt.legend()
plt.figure()
...
plt.xlabel("Epochs")
plt.ylabel("Error")
plt.title("Error vs. Epochs for two-hidden layers of {0} units".format(numHiddenUnits))
plt.legend()
